...
class Auth extends CI_Controller
{
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'noobiedevid@gmail.com',
            'smtp_pass' => 'ADSLSA5100',
            'smtp_port' => 465,
            'mailtype'  => 'html',
            'charset'   => 'utf-8',
            'newline'   => "\r\n"
        ...
        $this->email->from('noobiedevid@gmail.com', 'NBD ID');
        $this->email->to($this->input->post('email'));

        if ($type == 'verify') {
            $this->email->subject('Starter CMS | Account Verification');
            $this->email->message('Click this link to verify your account : 
                <a href="' . base_url() . 'auth/verify?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Active</a>
                ');
        } else if ($type == 'forgot') {
            $this->email->subject('Starter CMS | Reset Password');
    ...
    public function verify()
    {
        ...
        if ($user) {
            ...
            $this->session->set_flashdata('message', '
                <div class="alert alert-danger" role="alert">
                    Account activation failed! Wrong E-Mail
                </div>
            ');
            redirect('auth');
        }
    ...
    public function forgotPassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($user) {
                ...
                $this->session->set_flashdata('message', '
                <div class="alert alert-danger" role="alert">
                    Email is not registered / activated!
                </div>
                ');
                redirect('auth/forgotpassword');
            }
    ...
    public function changePassword()
    {
        ...
        $this->form_validation->set_rules('password1', 'Password', 'required|matches[password2]|min_length[6]');
